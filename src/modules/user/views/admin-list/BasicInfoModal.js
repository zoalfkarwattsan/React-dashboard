...
import {_addAdmin, _editAdminInfo, _getAllResponsiblesWithQ, _getAllRoles, _getAllRolesWithQ} from "../../redux/actions"
...
const BasicInfoModal = (props) => {
  ...
  useEffect(() => {
    _getAllRoles(
        data => {
          setRoleOptions(_.map(data, roleItem => ({label: roleItem.name, value: roleItem.id})))
        }
    )
  }, [])
    ...
    >
      <Form action='/' className='flex-grow-1 d-flex flex-column' onSubmit={handleSubmit(onSubmit)}>
        ...
        <ModalBody className='flex-grow-1 pb-sm-0 pb-3 modal-body'>
          ...
          <LoadingIndicator loading={_.size(roleOptions) <= 0}>
            <FormGroup>
              <Label className='form-label' for='rolesIDs'>
                {trans('user.role')}
              </Label>
              <Controller
                  as={Select}
                  ...
                  options={roleOptions}
                  className={classnames('react-select', { 'is-invalid': errors['rolesIDs'] || _.get(valErrors, 'rolesIDs') })}
                  defaultValue={_.get(props, 'data.userRoles') ? _.map(props.data.userRoles, x => _.find(roleOptions, a => a.value === x.roleId)) : ''}
                  rules={{
                    required: trans('user.validation.required')
                  }}
              />
              <ErrorMessages valErrors={valErrors} errors={errors} name={'rolesIDs'} />
            </FormGroup>
